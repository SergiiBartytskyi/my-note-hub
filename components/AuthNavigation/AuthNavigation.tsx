...
const AuthNavigation = () => {
  ...
  return isAuthenticated ? (
    ...
    <>
      {links.map(link => (
        <li key={link.href} className="w-full flex items-center justify-between gap-4">
          <Link
            href={link.href}
            className={clsx(
              'w-full whitespace-nowrap flex items-center justify-center rounded-xl border px-3 py-2 text-sm font-medium transition-colors focus:outline-none focus-visible:ring-2 focus-visible:ring-blue-600 dark:focus-visible:ring-blue-500',
              isActive(link.href)
